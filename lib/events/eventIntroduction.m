...
%% init
    function [elements, msg] = eventIntroduction_init(SDS, event)
        ...
        [P.fireTransmissionMSM, msg] = spTools('handle', 'eventTransmissionMSM', 'fire');
       % ******* Variables & Constants *******
...
%% fire
    function [SDS, P0] = eventIntroduction_fire(SDS, P0)
        ...
        if P0.index<=SDS.number_of_males;
            ...
            if P0.MSM(P0.male)
                P0.introduce = true;
                [SDS, P0]=P.fireTransmissionMSM(SDS, P0);
                P0.introduce = false;
            end
...
%% enable
    function  eventIntroduction_enable(SDS)
        ...
        for ti = P.rand
            if rand<=ratio
                age = ti-maleBorn;
                prob = (interp1q([15 25 45]',[0 0.26 0]',age(1:SDS.initial_number_of_males)'))';
                prob(isnan(prob)) = 0;
                ...
                age = ti-femaleBorn;
                prob = (interp1q([15 23 50]',[0 0.33 0]',age(1:SDS.initial_number_of_females)'))';
                prob(isnan(prob)) = 0;
                prob = cumsum(prob/sum(prob));
                infectedIdx = min(find(prob>rand));
