function demographicGraphs(SDS)
...
t =floor(max(SDS.relations.time(~isinf(SDS.relations.time(:,1)),1))-min(SDS.relations.time(~isinf(SDS.relations.time(:,1)),1)));
...
alive(i) = sum(born<i&death>i);
prev(i) = sum(timeHIVpos<i&death>i)/alive(i);
adult = born<(i-15)&born>(i-50)&death>i;
adultPop(i)  = sum(adult);
adultPrev(i) =  sum(timeHIVpos<i&death>i&adult)/adultPop(i);
relations(i) = sum(SDS.relations.time(:,1)<i&SDS.relations.time(:,2)>i)/adultPop(i);
arv(i) = sum(SDS.ARV.time(:,1)<i&(isnan(SDS.ARV.time(:,2))|SDS.ARV.time(:,2)>i))/sum(timeHIVpos<i&death>i&adult);
end
...
subplot(5,2,1); plot(table.time,table.alive); title('Population Size');
subplot(5,2,2); plot(table.time,table.concurrency); title('Average Concurrent Relationships');
subplot(5,2,3); plot(table.time,table.prevalence); title('Prevalence');
subplot(5,2,4); plot(table.time,table.adult_prev); title('Prevalence in Adults (>= 15 yrs)');
subplot(5,2,5); hist(table.pregnant_age); title('Age at children delivery');
subplot(5,2,6); hist(t-born,20); title('Age distribution at the end of simulation');
subplot(5,2,7); hist(table.age_at_infection,20);title('Age distribution at infection');
subplot(5,2,8);hist([SDS.males.HIV_positive SDS.females.HIV_positive],30); title('New infections');
subplot(5,2,9); plot(table.time,adultPop);title('adult pop');
subplot(5,2,10);plot(table.time,arv);title('arv');
